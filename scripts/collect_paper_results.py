...
def collect_classification(manifest: Path, out_root: Path, strict: bool) -> tuple[list[dict], list[dict]]:
    per_run: list[dict] = []
    missing: list[str] = []
...
def collect_survival(manifest: Path, out_root: Path, strict: bool) -> tuple[list[dict], list[dict]]:
    per_run: list[dict] = []
    missing: list[str] = []
    for row in read_tsv(manifest):
...
def main() -> None:
    ...
    parser.add_argument("--classification-manifest", type=Path, default=Path("configs/paper_classification.tsv"))
    parser.add_argument("--survival-manifest", type=Path, default=Path("configs/paper_tcga_survival.tsv"))
    parser.add_argument("--classification-runs", type=Path, default=Path(os.environ.get("GATEDSRP_CLASSIFICATION_OUT", "runs/classification")))
    parser.add_argument("--survival-runs", type=Path, default=Path(os.environ.get("GATEDSRP_TCGA_SURVIVAL_OUT", "runs/tcga_survival_global_seed_main")))
    ...
    class_per, class_summary = collect_classification(args.classification_manifest, args.classification_runs, args.strict)
    surv_per, surv_summary = collect_survival(args.survival_manifest, args.survival_runs, args.strict)

    class_fields = sorted({key for row in class_per for key in row})
    surv_fields = sorted({key for row in surv_per for key in row})
    write_tsv(args.out_dir / "classification_per_run.tsv", class_per, class_fields)
    write_tsv(args.out_dir / "classification_summary.tsv", class_summary, sorted({key for row in class_summary for key in row}))
    write_tsv(args.out_dir / "tcga_survival_per_run.tsv", surv_per, surv_fields)
    write_tsv(args.out_dir / "tcga_survival_summary.tsv", surv_summary, sorted({key for row in surv_summary for key in row}))
